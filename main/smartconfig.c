...
static void event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    if (event_base == SC_EVENT && event_id == SC_EVENT_GOT_SSID_PSWD) {
        ...
        context->config.ssid = malloc(sizeof(evt->ssid));
        context->config.password = malloc(sizeof(evt->password));

        bzero(&wifi_config, sizeof(wifi_config_t));
        memcpy(wifi_config.sta.ssid, evt->ssid, sizeof(wifi_config.sta.ssid));
        memcpy(wifi_config.sta.password, evt->password, sizeof(wifi_config.sta.password));

        memcpy(context->config.ssid, evt->ssid, sizeof(evt->ssid));
        memcpy(context->config.password, evt->password, sizeof(evt->password));
        ESP_LOGI(TAG, "Found ssid: %s", context->config.ssid);
        ESP_LOGI(TAG, "Found password: %s", context->config.password);
